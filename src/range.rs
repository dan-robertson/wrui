...
macro_rules! int_range_index {
    ($(#[$attr:meta])* struct $Self_:ident($T:ty)) => (
        ...
        impl range::Int for $Self_ {
            #[inline]
            fn zero() -> $Self_ { $Self_(range::Int::zero()) }
            #[inline]
            fn one() -> $Self_ { $Self_(range::Int::one()) }
            #[inline]
            fn max_value() -> $Self_ { $Self_(range::Int::max_value()) }
            #[inline]
            fn from_usize(n: usize) -> Option<$Self_> { range::Int::from_usize(n).map($Self_) }
        }
